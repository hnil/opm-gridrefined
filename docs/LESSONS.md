...
12. **Reference-space refinement with the trilinear map is the right kernel.** It is correct for any hexahedron, including degenerate ones (corners of collapsed edges map to coincident points — only topology dedup is missing); mosaics on shared faces are automatically consistent because both sides restrict to the same bilinear surface; and fault overlap polygons in parameter space are *refinement-invariant*, hence computable once and cached (§8.1, §8.4).
