                                 ...
                                 bool isNested)
{
    ...
    // getMark() moved to the CpGrid facade in opm-gridrefined (CpGridData no
    // longer exposes it). The DUNE mark/adapt refinement path these checks
    // exercise is an inert stub here; only the addLgrs (builder) path is live.
    BOOST_CHECK_EQUAL( grid.getMark(origin), 1);
    BOOST_CHECK_EQUAL( grid.getMark(father), 1);
...
namespace {
// RAII registration of the static-refinement backend for the duration of an
// addLgrsUpdateLeafView() call. In opm-gridrefined the build algorithm lives
// behind the Opm::Refinement::Builder seam (docs/DESIGN-builder.md); the deck
// path self-registers a ConformingBlockBuilder from retained corner-point
// input only when the deck has CARFIN. These test helpers add LGRs
// programmatically (no deck CARFIN), so they register the same builder
// explicitly -- the test bodies stay unchanged.
struct TestBuilderGuard
{
    explicit TestBuilderGuard(std::unique_ptr<Opm::Refinement::Builder> b)
        : previous_{Opm::Refinement::setBuilder(std::move(b))}
    {}
    ~TestBuilderGuard() { Opm::Refinement::setBuilder(std::move(previous_)); }
    std::unique_ptr<Opm::Refinement::Builder> previous_;
};

// Corner-point arrays for a regular Cartesian box, identical to the grdecl
// CpGrid::createCartesian() builds internally (shift = 0), so the builder
// resamples exactly the geometry the grid was created from.
void regularCartesianGrdecl(const std::array<int,3>& dims,
                            ...
                            std::vector<int>& actnum)
{
    ...
    const double bot = 0.0;
    const double top = dims[2]*cellsize[2];
    for (int j = 0; j < dims[1] + 1; ++j) {
        const double y = j*cellsize[1];
        for (int i = 0; i < dims[0] + 1; ++i) {
            const double x = i*cellsize[0];
            const double pillar[6] = { x, y, bot, x, y, top };
            coord.insert(coord.end(), pillar, pillar + 6);
        }
    }
    zcorn.assign(8*dims[0]*dims[1]*dims[2], 0.0);
    const int num_per_layer = 4*dims[0]*dims[1];
    double* offset = zcorn.data();
    for (int k = 0; k < dims[2]; ++k) {
        std::fill_n(offset, num_per_layer, k*cellsize[2]);     offset += num_per_layer;
        std::fill_n(offset, num_per_layer, (k+1)*cellsize[2]); offset += num_per_layer;
    }
    actnum.assign(dims[0]*dims[1]*dims[2], 1);
}
} // anonymous namespace
                               ...
                               const std::vector<std::string>& lgr_name_vec)
{
    ...
    const std::array<int,3> dims{ static_cast<int>(eclipse_grid.getNX()),
                                  static_cast<int>(eclipse_grid.getNY()),
                                  static_cast<int>(eclipse_grid.getNZ()) };
    TestBuilderGuard guard(std::make_unique<Opm::Refinement::ConformingBlockBuilder>(
        dims, eclipse_grid.getCOORD(), eclipse_grid.getZCORN(), eclipse_grid.getACTNUM()));
                               ...
                               const std::vector<std::string>& lgr_name_vec)
{
    ...
    std::vector<int> actnum;
    regularCartesianGrdecl(grid_dim, cell_sizes, coord, zcorn, actnum);
    TestBuilderGuard guard(std::make_unique<Opm::Refinement::ConformingBlockBuilder>(
        grid_dim, std::move(coord), std::move(zcorn), std::move(actnum)));
    grid.addLgrsUpdateLeafView(cells_per_dim_vec, startIJK_vec, endIJK_vec, lgr_name_vec);
}
                              ...
                              const std::vector<int>& markedCells)
{
    ...
    // refineAndUpdateGrid() (the old DUNE mark/adapt kernel) was removed in
    // opm-gridrefined; this mark-path helper is dead. Refinement now runs
    // through addLgrsUpdateLeafView() + the Refinement::Builder seam.
    OPM_THROW(std::logic_error, "adaptGridWithParams: the DUNE mark/adapt refinement path is not available in opm-gridrefined; use createGridAndAddLgrs.");
